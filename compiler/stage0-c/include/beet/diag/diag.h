...
typedef struct beet_diag {
    ...
    const char *message;
    const char *note;
} beet_diag; // Represents a diagnostic message, including its severity level, source location, and message text

typedef struct beet_diag_options {
    bool use_color;
} beet_diag_options; // Options for controlling the formatting of diagnostic messages

void beet_diag_emit(
    FILE *stream,
    const beet_diag *diag,
    beet_diag_options options
); // Emits a diagnostic message to the specified output stream, formatted according to the provided options
