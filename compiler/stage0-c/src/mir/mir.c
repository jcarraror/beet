...
void beet_mir_function_init(beet_mir_function *function, const char *name, size_t name_len) {
...
int beet_mir_lower_binding(
    ...
    if (!beet_mir_add_bind_local(function, binding->name, binding->name_len, temp)) {
        return 0;
    }
...
int beet_mir_lower_trivial_return_function(
    ...
    return beet_mir_add_return_const_int(function, return_value);
}